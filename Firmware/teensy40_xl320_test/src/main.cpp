...
StatusPacket* receiveStatus(HardwareSerial* serial, uint16_t param_length){
    ...
    if((*serial).available() >= total_length){
        ...
        if(buf[total_length-2] != crc_value.crc_8[0] || buf[total_length-1] != crc_value.crc_8[1]){
            return nullptr; //bad CRC
        }else{
            StatusPacket packet;
            packet.error = buf[8];
            packet.param_length = (buf[6] << 8) + buf[5];
            packet.packet_id = buf[4];
        }
    }else{
        return nullptr; //message length expected not available
    }
}
...
int updateServoObjects(StatusPacket packet, ServoXL320** servo_list, int len){
    ...
    int length = ((packet.lengthH << 8) + packet.lengthL);
    int packet_length = length - 9;
    uint8_t params[packet_length] = packet
    crc_combine crc_value = update_crc(0, );
}
